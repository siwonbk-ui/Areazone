...
(async()=>{
  ...
  try {
    ...
    await page.locator('#hazardSeg [data-hazard=flood]').click();
    await page.locator('.p-head').first().click();
    const evidence=page.locator('.p-card.open .ev-details').first();
    assert(await evidence.isVisible());
    assert.equal(await evidence.getAttribute('open'),null);
    await evidence.locator('summary').click();
    assert(await evidence.locator('.ev-content').isVisible());
    const evidenceText=await evidence.innerText();
    assert(!/แผ่นดินไหว|วาตภัย|พายุฤดูร้อน|ดินโคลนถล่ม|แล้ง/.test(evidenceText));
    assert.equal(await page.locator('.p-card.open .d-row').first().locator('.d-dot-wrap').count(),1);
